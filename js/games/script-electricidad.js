...
const LEVELS = [
  {
    ...
    solution: {
      ...
      validation: (circuit) => {
        // Debe haber fase -> interruptor -> bombilla
        // Y neutro -> bombilla
...
// ============================================
let currentLevel = 0;
let placedComponents = []; // Componentes colocados en el circuito
let wires = []; // Cables conectados
let draggedComponent = null;
let dragOffset = { x: 0, y: 0 };
let selectedWireType = null;
let wireStart = null;
let wirePreview = null; // Para mostrar preview del cable
let hoveredComponent = null; // Para resaltar componentes al pasar el mouse
let score = 0;
...
// ============================================
// DIBUJO - PANEL DE COMPONENTES (IZQUIERDA)
// ============================================
function drawToolbar() {
  ...
  // Dibujar componentes disponibles
  let y = area.y + 50;
  ctx.textAlign = 'left';
  ctx.font = '13px Arial';
...
function drawWorkspace() {
  ...
  // Preview del cable mientras se dibuja
  ...
  // Resaltar puntos de conexión cuando se está dibujando un cable
...
function drawComponent(comp) {
  ...
  if(comp.type === 'SOURCE_FASE') {
    ...
    // Puntos de conexión
    drawConnectionPoint(comp.x - 25, comp.y, '#8B0000'); // Entrada
    drawConnectionPoint(comp.x + 15, comp.y - 20, '#000'); // Salida 1
    drawConnectionPoint(comp.x + 15, comp.y + 20, '#000'); // Salida 2
    ...
    // Puntos de conexión
    drawConnectionPoint(comp.x - 15, comp.y + 20, '#8B0000'); // Fase
    drawConnectionPoint(comp.x + 15, comp.y + 20, '#0000CD'); // Neutro
  }
...
// ============================================
// DIBUJO - PANEL DE INFORMACIÓN (DERECHA)
// ============================================
...
function testCircuit() {
  ...
  // Validación nivel 1 y 2: Circuito básico
  if(currentLevel <= 1) {
    const hasFase = counts.faseSource > 0;
    const hasNeutro = counts.neutroSource > 0;
    const hasSwitch = counts.switchSimple > 0;
    ...
    // Verificar que el interruptor esté en la fase
    ...
    // Simular: si interruptor está ON, la luz se enciende
    if(switchComp.on) {
  ...
  if(currentLevel === 2) {
    ...
    // Verificar que ambos conmutadores estén en la misma posición (simulación)
  ...
  // Validación nivel 4: Cruzamiento
  if(currentLevel === 3) {
    const conmutadores = placedComponents.filter(c => c.type === 'CONMUTADOR');
...
function snapToGrid(x, y) {
  ...
  // Calcular posición relativa dentro del workspace
...
// Obtener puntos de conexión de un componente
...
function handleClick(e) {
  ...
  if(gameState === 'menu') {
    ...
    // Seleccionar componente de la toolbar
    ...
    for(const compKey of level.availableComponents) {
      if(isInsideRect(pos.x, pos.y, toolArea.x + 10, y, toolArea.w - 20, 60)) {
        // Crear nuevo componente para arrastrar
        const comp = COMPONENT_DEFS[compKey];
    ...
    if(isInsideRect(pos.x, pos.y, wsArea.x, wsArea.y, wsArea.w, wsArea.h)) {
      // Clic en componente para toggle (interruptores)
      for(const comp of placedComponents) {
      ...
      // Si hay un componente siendo arrastrado, soltarlo con snap
      if(draggedComponent) {
      ...
      // Si hay un tipo de cable seleccionado, dibujar cable
      if(selectedWireType) {
        ...
        if(!wireStart) {
          ...
          // Detectar componentes cerca de los puntos de conexión para validación
          let fromType = null;
          let toType = null;
          
          for(const comp of placedComponents) {
            // Verificar múltiples puntos de conexión según el tipo de componente
            if(comp.type === 'SOURCE_FASE' || comp.type === 'SOURCE_NEUTRO') {
...
function handleMouseMove(e) {
  ...
  // Detectar componente bajo el cursor
